...
Reference: Section 4, Stage 4 of the concept paper.
"""
...
# ── Domain Rules (Section 3.4 of concept paper) ───────────────────────────────
DOMAIN_RULES = """
...
_JOIN_CLAUSE_RE = re.compile(
    ...
    r'(?:\s+ON\s+(?P<on_clause>.+?))?'       # ON … (greedy up to next join/clause)
...
def _column_references_outside_join(sql: str) -> set[str]:
    ...
    """
    # Remove every ON … sub-clause so join predicates don't count as usage
    stripped = re.sub(
