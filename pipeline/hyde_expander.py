...
class LLMHyDEExpander:
    ...
    def expand_or_raise(self, question: str) -> str:
        ...
        Layer 0 (deterministic):
            Checks the question against the concept map.  If the question
            contains zero known concepts AND no direct column/table name hits,
            the LLM concept-coverage check is invoked.

        Layer 1 (token presence):
            After LLM expansion, if the cleaned passage has no known column
            tokens and the raw question also has no direct column hits → raise.

        Layer 2 (LLM concept coverage):
            Ask the LLM to enumerate queryable concepts and verify each
            against the schema.  Unmapped concepts → raise.
        """
    ...
    def _ontology_precheck(self, question: str) -> Optional[List[str]]:
        ...
        Works for any schema because the concept map is built from the registry,
        not a static dict.  The _UNMAPPABLE list has been replaced by the LLM
        concept coverage check (Layer 2), which is schema-agnostic.
        """
        ...
        # (returning None lets expand_or_raise reach Layer 1 & 2)
        return None
    ...
    def _check_concept_coverage(self, question: str) -> List[str]:
        ...
        This check is schema-agnostic: the schema block passed to the LLM
        always reflects the current registry, so it scales automatically.
        """
        q_lower = question.lower()
