...
def _render_trace_panel(last_r, last_q: str) -> None:
    ...
    with st.expander("Query Trace", expanded=True):
        ...
        # ── Stage 5: Self-Healing Repair History ───────────────────────────────
        repair_history = getattr(t, "repair_history", []) or []
        if repair_history:
            st.markdown(
                f"**Stage 5 — Self-Healing Repair History** "
                f"({len(repair_history)} repair attempt{'s' if len(repair_history) != 1 else ''}):"
            )
            for i, r in enumerate(repair_history, 1):
                with st.expander(
                    f"Attempt {i} failed — {r.get('error', '')[:90]}"
                    f"{'...' if len(r.get('error', '')) > 90 else ''}",
                    expanded=(i == len(repair_history)),
                ):
                    st.markdown("*Failing SQL:*")
                    st.markdown(
                        f"<div class='sql-box'>{r.get('sql', '')}</div>",
                        unsafe_allow_html=True,
                    )
                    st.markdown("*Error:*")
                    st.markdown(
                        f"<div style='background:#fff5f5; border:1px solid #fca5a5; "
                        f"border-radius:6px; padding:0.6rem 0.9rem; color:#b91c1c; "
                        f"font-family:monospace; font-size:0.85rem;'>{r.get('error', '')}</div>",
                        unsafe_allow_html=True,
                    )
            st.caption(
                "The repair agent received the failing SQL and error above, and produced "
                "the corrected SQL shown below."
            )

        # ── Stage 4+5: Generated SQL ──────────────────────────────────────────
        st.markdown(
            f"**Stage 4+5 — {'Final Corrected' if repair_history else 'Generated'} SQL** "
